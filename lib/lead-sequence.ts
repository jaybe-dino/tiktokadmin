...
//   기존 1회성 자동안내(welcome.ts)의 확장:
//     · 1일차는 기존대로 유입 즉시(설정에서 "10시 발송"으로 바꿀 수 있음)
//     · 2일차부터는 매일 지정 시각(기본 오전 10시, KST)에 일차별로 다른 문구
//   멱등: 브랜드×일차 1회(lead_sequence_sends UNIQUE). 크론이 여러 번 돌아도 중복 발송 없음.
...
const clampHour = (n: unknown): number => Math.min(23, Math.max(0, Math.round(Number(n) || 0)));

/** 설정 조회 — 0096 미적용 DB 에서는 "꺼짐"으로 안전하게 동작. */
export async function getSeqSchedule(): Promise<SeqSchedule> {
  const r = await queryOne<{
    seq_enabled: boolean; seq_days: number; seq_hour: number;
    seq_day1_immediate: boolean; seq_skip_weekend: boolean; seq_stop_on_progress: boolean;
  }>(`SELECT seq_enabled, seq_days, seq_hour, seq_day1_immediate, seq_skip_weekend, seq_stop_on_progress
        FROM welcome_config WHERE id=1`).catch(() => null);
  return {
    enabled: !!r?.seq_enabled,
    days: clampDays(r?.seq_days ?? 7),
    hour: clampHour(r?.seq_hour ?? 10),
    day1Immediate: r?.seq_day1_immediate ?? true,
    skipWeekend: !!r?.seq_skip_weekend,
    stopOnProgress: r?.seq_stop_on_progress ?? true,
  };
}

export async function saveSeqSchedule(s: Partial<SeqSchedule>): Promise<void> {
  await query(
    `UPDATE welcome_config SET
       seq_enabled=COALESCE($1,seq_enabled), seq_days=COALESCE($2,seq_days), seq_hour=COALESCE($3,seq_hour),
       seq_day1_immediate=COALESCE($4,seq_day1_immediate), seq_skip_weekend=COALESCE($5,seq_skip_weekend),
       seq_stop_on_progress=COALESCE($6,seq_stop_on_progress), updated_at=now()
     WHERE id=1`,
    [s.enabled ?? null, s.days == null ? null : clampDays(s.days), s.hour == null ? null : clampHour(s.hour),
     s.day1Immediate ?? null, s.skipWeekend ?? null, s.stopOnProgress ?? null]);
}

/** 일차별 문구 — 저장된 행이 없는 일차는 빈 기본값으로 채워 항상 days 개를 돌려준다. */
export async function listSeqSteps(days: number): Promise<SeqStep[]> {
  const rows = await query<SeqStep>(
    "SELECT day_no, enabled, send_sms, send_email, sms_body, email_subject, email_body FROM lead_sequence_steps ORDER BY day_no",
  ).catch(() => []);
  const byDay = new Map(rows.map((r) => [r.day_no, r]));
  return Array.from({ length: clampDays(days) }, (_, i) => {
    const d = i + 1;
    return byDay.get(d) ?? { day_no: d, enabled: d === 1, send_sms: true, send_email: true, sms_body: "", email_subject: "", email_body: "" };
  });
}

export async function saveSeqStep(s: SeqStep, by: string): Promise<void> {
  await query(
    `INSERT INTO lead_sequence_steps (day_no, enabled, send_sms, send_email, sms_body, email_subject, email_body, updated_by)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
     ON CONFLICT (day_no) DO UPDATE SET enabled=EXCLUDED.enabled, send_sms=EXCLUDED.send_sms,
    ...
    [Math.min(MAX_SEQ_DAYS, Math.max(1, s.day_no)), s.enabled, s.send_sms, s.send_email,
...
/**
 * 일차별 예정 시각 목록. 1일차가 "즉시"면 유입 시각 그대로,
 * 아니면 유입 당일 지정 시각(이미 지났으면 다음 날)부터 시작한다.
 */
export function planSchedule(from: Date, s: SeqSchedule): { day_no: number; due_at: Date }[] {
...
// ── 등록(유입 시) ─────────────────────────────────────────────
/** 리드 유입 시 N일치 예약을 만든다. 이미 예약된 브랜드면 아무것도 하지 않는다. */
export async function enrollLead(brandId: string, from = new Date()): Promise<{ ok: boolean; scheduled: number; skipped?: string }> {
  const s = await getSeqSchedule();
  if (!s.enabled) return { ok: true, scheduled: 0, skipped: "연속 안내 비활성" };
  ...
  const plan = planSchedule(from, s);
  let n = 0;
  for (const p of plan) {
    const r = await query(
      `INSERT INTO lead_sequence_sends (brand_id, day_no, due_at) VALUES ($1,$2,$3)
       ON CONFLICT (brand_id, day_no) DO NOTHING`, [brandId, p.day_no, p.due_at.toISOString()]).catch(() => null);
    if (r) n++;
  }
...
// ── 발송(크론) ────────────────────────────────────────────────
// 단계가 진전되면 더 보내지 않는다 — 이미 대화가 시작된 상대에게 안내가 계속 나가지 않게.
...
export async function runDueSequence(limit = 200, now = new Date()): Promise<SeqRunResult> {
  ...
  const steps = new Map((await listSeqSteps(MAX_SEQ_DAYS)).map((x) => [x.day_no, x]));
  ...
  for (const r of rows) {
    ...
    const stopReason = r.msg_opt_out ? "수신거부"
      ...
      : s.stopOnProgress && r.state !== "lead_new" ? `단계 진전(${r.state})`
      : null;
    if (stopReason) {
    ...
    if (sent.length) {
      ...
      await query(
        ...
        [r.brand_id, JSON.stringify({ channel: sent.join("+"), kind: "lead_sequence", day: r.day_no })]).catch(() => {});
      res.sent++;
    } else {
...
export interface SeqQueueRow {
  brand_id: string; brand_name: string; day_no: number; due_at: string;
  status: string; channels: string[]; note: string;
}
/** 다가오는 예약·최근 발송 — 설정 화면에서 "언제 누구에게 나가는지" 확인용. */
export async function listSeqQueue(limit = 40): Promise<SeqQueueRow[]> {
  return query<SeqQueueRow>(
    `SELECT q.brand_id, b.brand_name, q.day_no, q.due_at::text AS due_at, q.status, q.channels, q.note
       FROM lead_sequence_sends q JOIN brands b ON b.id=q.brand_id
      WHERE q.status IN ('queued','sent','failed')
      ORDER BY (q.status='queued') DESC, q.due_at
      LIMIT $1`, [limit]).catch(() => []);
}

/** 1일차를 기존 자동안내(welcome)가 이미 보냈을 때 — 그 예약을 발송됨으로 닫아 중복을 막는다. */
export async function markDay1Sent(brandId: string, channels: string[]): Promise<void> {
  const s = await getSeqSchedule();
