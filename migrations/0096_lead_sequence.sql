...
--   회차는 day_no 로 센다: 1 = 유입 1일차, 2 = 2일차 …
--   ※ 유입 즉시 발송은 기존 1회성 자동안내(intake_channels 의 문자·메일 문구)가 그대로 담당한다.
--     여기서 또 보내면 같은 순간에 두 군데서 나가므로 1일차부터만 다룬다.
...
CREATE TABLE IF NOT EXISTS lead_sequence_config (
  ...
  days int NOT NULL DEFAULT 5 CHECK (days BETWEEN 1 AND 30),    -- 몇 일차까지
...
CREATE TABLE IF NOT EXISTS lead_sequence_steps (
  ...
  day_no int NOT NULL CHECK (day_no BETWEEN 1 AND 30),   -- 1 = 유입 1일차
  enabled boolean NOT NULL DEFAULT true,
  send_sms boolean NOT NULL DEFAULT true,
  send_email boolean NOT NULL DEFAULT true,
  send_hour int CHECK (send_hour BETWEEN 0 AND 23),  -- NULL 이면 키 기본 시각
