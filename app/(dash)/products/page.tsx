...
export default async function ProductsPage() {
  ...
  // 롤업 집계 (실데이터 기반)
  const totalCert = products.reduce((s, p) => s + ((p.cert_total as number) ?? 0), 0);
  const readyCert = products.reduce((s, p) => s + ((p.cert_ready as number) ?? 0), 0);
  ...
  const brandChips = [...new Set(products.map((p) => p.brand_name as string).filter(Boolean))].slice(0, 5);
    ...
    <div>
      <div className="ph">
        ...
        <div className="bar" style={{ margin: 0 }}>
          <input className="f" style={{ width: 210 }} placeholder="🔍 브랜드 검색 (계약된 업체)" />
          <select><option>계약 업체만</option><option>전체 브랜드</option></select>
          <select><option>국가 전체</option></select>
        </div>
      </div>
      ...
      {brandChips.length > 0 && (
        <div className="bar" style={{ marginTop: -6 }}>
          ...
          {brandChips.map((b) => (<span key={b} className="chip">{b}</span>))}
        </div>
      )}
      ...
      <div className="grid g31 gap-3.5">
        ...
        <div className="card">
          ...
          <table className="t matrix">
            ...
            <tbody>
              {products.length === 0 && (
                <tr><td colSpan={4} style={{ textAlign: "left", color: "var(--ink3)" }}>등록된 제품이 없습니다.</td></tr>
              )}
              {products.map((p) => {
          ...
          <div className="card">
            ...
            <div className="bd" style={{ fontSize: 12 }}>
              <div className="row">
                ...
                <div><div className="tt">제품 {products.length}종</div><div className="ss">계약 브랜드 제품 마스터</div></div>
              </div>
              <div className="row">
              ...
              <div className="row">
                <span className={`ico ${risks.length ? "i-red" : "i-grn"}`}>{risks.length ? "⚠️" : "🟢"}</span>
                <div><div className="tt">인증 리스크 {risks.length}건</div><div className="ss">만료·미비·반려 포함</div></div>
              </div>
            </div>
          </div>

          <div className="card">
            <div className="hd"><b>국가별 리스크</b>{risks.length > 0 && <span className="chip red">{risks.length}</span>}</div>
            <div className="bd" style={{ fontSize: 12 }}>
              {countryRisks.length === 0 && <div className="note">인증 리스크가 없습니다.</div>}
              {countryRisks.map(([c, n]) => (
                <div className="row" key={c}>
                  <span className="ico i-amb">{flag(c) || "🌐"}</span>
                  <div><div className="tt">{flag(c)} {c}</div><div className="ss">리스크 {n}건</div></div>
      ...
      <div className="card" style={{ marginTop: 14 }}>
        <div className="hd">
          ...
          <div className="rt"><span style={{ color: "var(--ink3)", fontSize: 11 }}>만료·미비 자료 {risks.length}건</span></div>
        </div>
        <table className="t">
          <thead>
