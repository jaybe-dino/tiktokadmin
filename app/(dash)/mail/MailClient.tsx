...
// 메일함 3분할 클라이언트 — 스레드 선택 상태(useState)만 담당.
...
export default function MailClient({
  ...
  const sel = threads.find((t) => t.thread_id === selectedId) ?? threads[0] ?? null;
  ...
  return (
    ...
    >
          ...
          return (
            ...
            >
              ...
              <div style={{ fontSize: 12, marginTop: 1, display: "flex", alignItems: "center", gap: 5 }}>
                <span
                  className={t.direction === "out" ? "chip grn" : "chip"}
      ...
      <div style={{ padding: "16px 18px", overflowY: "auto", maxHeight: 640 }}>
        {sel ? (
          <>
            ...
            {selMsgs.length === 0 ? (
              <div className="note">이 스레드의 개별 메시지가 수집되지 않았습니다.</div>
            ) : (
                ...
                {selMsgs.map((m, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <span>
                          <b style={{ color: out ? "var(--acc)" : "var(--ink)" }}>
                            {out ? "발신" : "수신"}
                          </b>{" "}
                          · {m.from_addr ?? "발신자 미상"}
      ...
      {/* 오른: 브랜드 컨텍스트 + 답장 초안(표시만) */}
      <div style={{ borderLeft: "1px solid var(--line)", padding: 14, background: "#fcfaff" }}>
        {sel && (
          <>
            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 11.5, color: "var(--ink3)", fontWeight: 700, marginBottom: 6 }}>
                브랜드 컨텍스트
              </div>
              {sel.brand_id ? (
                <>
                  ...
                  <dl className="kv">
                    ...
                    <dd>
                      {sel.plan ? (
                        <PlanBadge plan={sel.plan as Plan} />
                      ) : (
                        <span style={{ color: "var(--ink3)" }}>미정</span>
                      )}
                    </dd>
                    <dt>결제</dt>
                    <dd>
                      {sel.pay_status ? (
                        <PayBadge status={sel.pay_status} />
                      ) : (
            ...
            <div style={{ display: "flex", gap: 6, marginTop: 10 }}>
              <button className="btn" style={{ flex: 1 }} type="button" disabled>
                ✏️ 답장 초안
              </button>
            </div>
